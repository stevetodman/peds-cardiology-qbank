...
2. **Use the sticky navigation bar, press `/` for instant filtering, or hit `Ctrl+K` / `⌘K` to launch the command palette.** The palette lets you jump to sections, copy their content, or pin them without scrolling.
3. **Pin the sections you revisit most often.** Click the 📌 buttons or use the command palette to add panels to the "Pinned Shortcuts" hub for one-click access when you return later.
4. **Toggle light or dark mode** from the navigation bar to match your lighting conditions.
5. **Skim the "Quick Start Workflow"** for a bird's-eye view of the entire production process, then mark progress with the Workflow Progress Tracker right below it.
6. **Follow the workflow panels** in order; each one mirrors the steps you would take when generating and validating new MCQs.
7. **Save approved JSON arrays** inside the JSON Batch Library so you can reopen, export, or re-import them without digging through folders.
...
| **Pinned Shortcuts** | Stores the panels you pin so they surface near the top of the page for fast reuse. |
...
* **Command palette:** Press `Ctrl+K` (Windows/Linux) or `⌘K` (macOS) to open a quick launcher that jumps, copies, or pins sections without leaving the keyboard.
* **Pinned shortcuts hub:** Your pinned panels surface near the top of the page so the most-used prompts are always one click away.
...
* **Share your pins:** Ask each role (writer, reviewer, PM) to pin their most-used sections so the "Pinned Shortcuts" list reflects the next task when someone else opens the file on the same machine.
