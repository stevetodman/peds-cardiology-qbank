...
6. **Save approved JSON arrays** inside the JSON Batch Library so you can reopen or export them later without digging through folders.
...
| **JSON Batch Library** | Lets you paste validated JSON, save it with a friendly title and notes, then reopen, export, or delete batches directly in the browser. |
...
* **JSON Batch Library:** Store, preview, and export JSON batches using built-in IndexedDB storage—no login or external software required.
...
* **Catalog approved JSON:** Save each vetted batch inside the JSON library with a short note (e.g., “Ready for SME review”) so teammates know the next action at a glance.
...
| JSON library buttons are disabled | Switch to a modern browser (Chrome, Edge, Firefox, Safari) or enable local storage. The JSON Batch Library needs IndexedDB access to save batches. |
...
Everything stays inside your browser using IndexedDB. Nothing is uploaded to a server, and you can export or delete batches at any time using the buttons in the JSON Batch Library.
